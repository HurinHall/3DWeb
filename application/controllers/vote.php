    ...
    $data['result'] = $query->result();*/
      ...
      // check if any race
          $data['result']=array();
           ...
           $nterm = "";
           $vs_datetime="";
           $ve_datetime="June 7, 2087 15:03:25";
           $racetitle="";
            ...
            foreach($query->result() as $item){
              $raceid= $item->id;
              $nterm = $item->nterm;
              $vs_datetime=$item->vs_datetime;
              $ve_datetime=$item->ve_datetime;
              $racetitle=$item->title;
            }
           ...
           if(strlen($raceid)>0){
              ...
              $data['result'] = $this->Work_model->vote_get_all_entries($config['per_page'],$this->uri->segment(3),$nterm);
              $data['pagelinks'] = $this->pagination->create_links();//创分页链接
           }

          $data['raceid'] = $raceid;
          $data['nterm'] = $nterm;
          $data['vs_datetime'] = $vs_datetime;
          $data['ve_datetime'] = $ve_datetime;
          $data['racetitle'] = $racetitle;
